...
def main():
    ...
    with open(RESULT_FILE_PATH, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        csvwriter.writerow(
            ['request_time', 'origin', 'destination', 'origin addresses', 'destination addresses', 'mode',
             'traffic_model', 'Departure time', 'Distance (meters)', 'Distance (text)', 'Duration (seconds)',
             'Duration (text)', 'Duration in traffic (seconds)', 'Duration in traffic (text)'])
        ...
        for t in data:
            ...
            print("%s) %s -> %s : [dist: %s] <> [dur: %s]" % (
            n, t['origin'], t['destination'], dm_distance['text'], dm_duration_in_traffic['text']))
