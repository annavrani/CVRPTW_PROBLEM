...
def calculate_penalty(violation):
    """
    Calculate penalty based on the violation.
    Assume a linear relationship between violation and penalty.
   """

    # Define parameters for the penalty function
    max_penalty = 2  # Maximum penalty value
    max_violation = 30 # Maximum violation

    # Calculate penalty
    penalty = min(violation, max_violation) / max_violation * max_penalty
    penalty = int(penalty)
    return penalty
...
def print_solution(data, manager, routing, solution):
    ...
    for vehicle_id in range(data["num_vehicles"]):
        ...
        while not routing.IsEnd(index):
            ...
            plan_output += '{0} (E:{1}, L:{2}, D:{3}) -> '.format(
...
def main():
    ...
    for location_idx, time_window in enumerate(data["time_windows"]):
        ...
        violation = cumul_var.Max() - time_window[1]
        penalty = calculate_penalty(violation)
